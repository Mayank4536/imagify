...
const Login = () => {
  ...
  //You cannot scroll the webpage when we open login page through this function
  useEffect(()=>{
     ...
     {/*<div className=' absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>*/}
      ...
      <motion.form onSubmit={onSubmitHandler} className=' relative bg-white p-10 rounded-xl text-slate-500'
       ...
       >
        ...
        <button type='submit' className=' bg-blue-600 w-full text-white py-2 rounded'>{state === 'Login' ? 'Login' : 'create account'}</button>

        {state === 'Login' ?
          <p className=' mt-5 text-center'>Don't have an account? <span onClick={()=>setState('Sign Up')} className=' text-blue-600 cursor-pointer'>SignUp</span></p>
          :
